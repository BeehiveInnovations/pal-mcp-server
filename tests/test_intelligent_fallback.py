...
class TestIntelligentFallback:
    ...
    def test_prefers_openai_when_both_available(self):
        """Test that Gemini is preferred when both API keys are available (based on provider priority)"""
        ...
        )  # Gemini Flash 3 Preview has priority (highest intelligence score)
    ...
    def test_auto_mode_with_gemini_only(self):
        ...
        with (
            ...
            with patch("utils.model_context.ModelContext") as mock_context_class:
                ...
                # Should use gemini-3-flash-preview when only Gemini is available (highest intelligence score)
